...
describe('LedgerService', () => {
  it('splits 10% platform commission from 10_000 UZS', () => {
    const s = new LedgerService(
      {
        get: (k: string, def: number) => (k === 'PLATFORM_COMMISSION_BPS' ? 1000 : def),
  ...
  it('writes a top-up ledger row and updates cached balance in one transaction scope', async () => {
    const s = new LedgerService({ get: (_k: string, def: number) => def } as unknown as ConfigService, {} as PrismaService);
    const tx = {
      driver: {
        findUnique: jest.fn().mockResolvedValue({ balanceUzs: new Prisma.Decimal(5_000) }),
  ...
  it('normalizes manual negative adjustments to MANUAL_ADJUSTMENT_MINUS with positive ledger amount', async () => {
    ...
    const tx = {
      driver: {
        findUnique: jest.fn().mockResolvedValue({ balanceUzs: new Prisma.Decimal(20_000) }),
        update: jest.fn().mockResolvedValue({}),
      },
      earningsLedger: {
