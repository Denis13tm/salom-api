...
export class DriverOrdersController {
  ...
  @Post(':orderId/reject')
  reject(@Param('orderId') orderId: string, @Req() req: Request, @Body() body: RejectOrderDto) {
    return this.dispatch.rejectOrder(orderId, req.salomDriverId!, body.rejectNote);
  }

  @Post(':orderId/cancel')
  cancel(@Param('orderId') orderId: string, @Req() req: Request, @Body() body: CancelOrderDto) {
    return this.lifecycle.cancelByDriver(orderId, req.salomDriverId!, body);
  }

  @Post(':orderId/passenger-no-show')
  passengerNoShow(@Param('orderId') orderId: string, @Req() req: Request, @Body() body: CancelOrderDto) {
    return this.lifecycle.markPassengerNoShow(orderId, { type: 'driver', driverId: req.salomDriverId! }, body);
  }
}
