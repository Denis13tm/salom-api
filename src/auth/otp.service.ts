...
export class OtpService {
  ...
  private pilotFixedOtp(): boolean {
    return isPilotFixedOtpEnabled(this.config.get<string | undefined>('PILOT_OTP_FIXED_ENABLED'));
  }
  ...
  private usePlaceholderOtpCode(): boolean {
    return this.pilotFixedOtp() || (this.devAuthBypass() && !this.realSmsMode());
  }
  ...
  async requestDriverOtp(phone: string) {
    if (!this.isEnabled()) {
      throw new ForbiddenException('OTP login o‘chirilgan (OTP_LOGIN_ENABLED=false)');
    }
    ...
    if (!this.pilotFixedOtp()) {
      ...
      await this.sms.sendToCustomer(null, dial, body, { failIfUndelivered: true });
    } else {
      this.log.warn(
    ...
    const showOtpHint = this.pilotFixedOtp() || (isDev && this.devAuthBypass() && !this.realSmsMode());
    return {
      requestId: row.id,
      expiresInSec: Math.max(30, Math.floor((expiresAt.getTime() - Date.now()) / 1000)),
      ...(showOtpHint ? { devOtp: code } : {}),
    };
  }

  async verifyDriverOtp(requestId: string, code: string): Promise<ExchangeAndRefreshResponse> {
    if (!this.isEnabled()) {
  ...
  private registrationOtpEnabled(): boolean {
    return this.config.get<string>('DRIVER_REGISTRATION_OTP', 'true') === 'true';
  }
  ...
  async requestRegistrationOtp(phone: string) {
    if (!this.registrationOtpEnabled()) {
      throw new ForbiddenException('Ro‘yxatdan kirish SMS o‘chirilgan (DRIVER_REGISTRATION_OTP=false)');
    }
    const driverId = await this.findDriverIdByPhone(phone);
    if (!driverId) {
      throw new NotFoundException('Avval "Ro‘yxatdan o‘tish" bosqichida telefoningizni qo‘shing');
    }
    ...
    if (!okOnboarding.includes(d.onboardingStatus)) {
      throw new ForbiddenException('Ariza allaqachon yopilgan — oddiy kirish (OTP) ishlating');
    }
    const code = this.usePlaceholderOtpCode()
      ? DEV_SMS_OTP_PLACEHOLDER
      : String(100_000 + randomInt(0, 900_000));
  ...
  async verifyRegistrationOtp(requestId: string, code: string): Promise<ExchangeAndRefreshResponse> {
    if (!this.registrationOtpEnabled()) {
