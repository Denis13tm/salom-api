...
export class AuthService {
  ...
  authStatus() {
    ...
    const smsMode = (this.config.get<string>('SMS_MODE') ?? 'log').toLowerCase();
    const smsHttpUrl = this.config.get<string>('SMS_HTTP_URL')?.trim();
    /** `log` / `stub` — haqiqiy operator tarmog‘iga SMS ketmaydi. */
    const realSmsToPhone = smsMode === 'eskiz' || (smsMode === 'http' && Boolean(smsHttpUrl));
    const pilotFixedOtp = isPilotFixedOtpEnabled(this.config.get<string | undefined>('PILOT_OTP_FIXED_ENABLED'));
    return {
      jwt: true as const,
      exchangeSecretRequired: Boolean(this.config.get<string>('SALOM_EXCHANGE_SECRET')),
      ...
      devAuthBypass,
      devSmsOtpPlaceholder: devAuthBypass && !realSmsToPhone ? DEV_SMS_OTP_PLACEHOLDER : null,
      devActivation12Placeholder: devAuthBypass ? DEV_ACTIVATION_12_PLACEHOLDER : null,
      adminWebPasswordConfigured: Boolean(this.config.get<string>('ADMIN_WEB_PASSWORD')?.trim()),
   ...
   */
  async loginAdminWebPassword(password: string): Promise<ExchangeAndRefreshResponse & { adminId: string }> {
    ...
    if (!admin) {
      throw new NotFoundException('Bazada admin yozuvi yo‘q — seed / migratsiya tekshiring');
    }
    const tokens = await this.issueWithNewRefresh(admin.userId, ADMIN_ROLE, admin.id);
  ...
  async loginOperatorPassword(
    ...
    const tokens = await this.issueWithNewRefresh(user.id, OPERATOR_ROLE, user.operator.id);
  ...
  private assertExchangeSecret(provided: string | undefined) {
    const expected = this.config.get<string | undefined>('SALOM_EXCHANGE_SECRET');
    if (expected) {
    ...
    if (this.config.get('NODE_ENV') === 'production') {
      throw new ForbiddenException('Set SALOM_EXCHANGE_SECRET for token exchange in production');
    }
  }

  async exchangeDriverToken(driverId: string, exchangeSecret: string | undefined) {
    ...
    if (d.activationCode && !d.appActivatedAt) {
      throw new ForbiddenException('Avval 12 xonali faollashtirish kodi (telefon) bilan tasdiqlang');
    }
    return this.issueDriverTokensById(driverId);
  }

  /** Driver JWT + refresh (OTP yoki alohida tasdiqdan keyin). */
  async issueDriverTokensById(driverId: string): Promise<ExchangeAndRefreshResponse> {
    const d = await this.prisma.driver.findUnique({
      where: { id: driverId },
      select: { id: true, userId: true },
    });
   ...
   */
  async activateDriverWithCode(phoneRaw: string, codeRaw: string): Promise<ExchangeAndRefreshResponse> {
    const expect = normalizePhoneUz(phoneRaw);
    const code = codeRaw.replace(/\D/g, '');
    if (code.length !== 12) {
      throw new BadRequestException('Faollashtirish kodi 12 raqam bo‘lishi kerak');
    }
    if (
      isDevAuthBypassEnabled(
        this.config.get<string | undefined>('NODE_ENV'),
    ...
    if (!d.activationCode) {
      throw new BadRequestException('Aktivatsiya kodi talab etilmaydi (legacy hisob)');
    }
    if (d.appActivatedAt) {
  ...
  private async findOrCreateDevDriverForPlaceholder(expect: string) {
    ...
    if (existing) {
      if (existing.role !== UserRole.DRIVER) {
        throw new UnauthorizedException('Bu telefon boshqa roldan foydalanilmoqda (seed/dev)');
      }
      if (existing.driver) {
  ...
  async exchangeOperatorToken(operatorId: string, exchangeSecret: string | undefined) {
    this.assertExchangeSecret(exchangeSecret);
  ...
  async exchangeAdminToken(adminId: string, exchangeSecret: string | undefined) {
    this.assertExchangeSecret(exchangeSecret);
  ...
  async refreshTokens(refreshTokenPlain: string): Promise<ExchangeAndRefreshResponse> {
  ...
  async logoutWithRefreshToken(refreshTokenPlain: string): Promise<{ ok: true }> {
    const tokenHash = hashRefreshToken(refreshTokenPlain);
    const row = await this.prisma.authRefreshSession.findFirst({
  ...
  async logoutAllSessions(authorizationHeader: string | undefined): Promise<{ ok: true; revoked: number }> {
    ...
    if (payload.role !== DRIVER_ROLE && payload.role !== OPERATOR_ROLE && payload.role !== ADMIN_ROLE) {
      throw new ForbiddenException();
    }
  ...
  private async resolveUserId(role: string, subjectId: string): Promise<string | null> {
    if (role === DRIVER_ROLE) {
      const d = await this.prisma.driver.findUnique({
        where: { id: subjectId },
        select: { userId: true },
  ...
  private async issueWithNewRefresh(
    ...
    const { plain, refreshExpiresInSec } = await this.createRefreshSession(userId, subjectId, role);
  ...
  private signAccess(
    ...
    const accessToken = this.jwt.sign(
      { sub, role, typ: 'access' },
      { expiresIn } as import('@nestjs/jwt').JwtSignOptions,
    );
    return { accessToken, tokenType: 'Bearer', expiresInSec: this.approxSeconds(expiresIn) };
  }
