...
import { AdminCreateVehicleDto, AdminPatchVehicleDto } from './dto/admin-driver-vehicle.dto';
...
export class AdminController {
  ...
  async adminLeaderboard(
    ...
    try {
      ...
      throw new BadRequestException(typeof msg === 'string' && msg.trim() ? msg : 'Leaderboard query failed');
    }
  }
  ...
  upsertMonthlyLeaderboardOverride(
    ...
    const now = new Date();
    const ymRaw =
      body?.periodYm?.trim() || `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, '0')}`;
  ...
  @HttpCode(200)
  patchDriverXpSettings(@Body() body: PatchDriverXpSettingsDto, @Req() req: Request) {
    return this.admin.patchDriverXpGamificationSettings(req.salomAdminUserId, body);
  }
  ...
  async driverXpLeaderboard(
    ...
    try {
      ...
      const msg = (e as { message?: unknown })?.message;
      throw new BadRequestException(typeof msg === 'string' && msg.trim() ? msg : 'XP leaderboard query failed');
    }
  }
  ...
  upsertDriverXpOverride(
    ...
    return this.admin.upsertDriverLifetimeXpOverrideAdmin(req.salomAdminUserId, driverId, body.xp);
  }

  @Delete('gamification/xp-override/:driverId')
  @HttpCode(200)
  deleteDriverXpOverride(@Param('driverId') driverId: string, @Req() req: Request) {
    return this.admin.deleteDriverLifetimeXpOverrideAdmin(req.salomAdminUserId, driverId);
  }
  ...
  syncSettlements(@Body() body: { periodYm: string }, @Req() req: Request) {
    return this.admin.persistMonthlySettlements(body.periodYm, req.salomAdminUserId);
  }
  ...
  async exportDaily(@Query('days') days: string | undefined, @Res() res: Response) {
    const d = days ? parseInt(days, 10) : 7;
    const n = Number.isNaN(d) ? 7 : d;
    const csv = await this.admin.exportDailyOrderStatsCsv(n);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="salom-daily-orders.csv"');
    return res.send(csv);
  }

  @Get('reports/pilot.csv')
  async exportPilot(@Query('days') days: string | undefined, @Res() res: Response) {
    const d = days ? parseInt(days, 10) : 14;
    const n = Number.isNaN(d) ? 14 : d;
    const csv = await this.admin.exportPilotReportCsv(n);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="salom-pilot-report.csv"');
    return res.send(csv);
  }
  ...
  drivers(
    ...
    if (accountStatus && Object.values(UserAccountStatus).includes(accountStatus as UserAccountStatus)) {
      ac = accountStatus as UserAccountStatus;
    }
    let ob: DriverOnboardingStatus | undefined;
    if (
      onboardingStatus &&
      Object.values(DriverOnboardingStatus).includes(onboardingStatus as DriverOnboardingStatus)
    ) {
  ...
  @HttpCode(200)
  deleteDriverDocument(@Param('driverId') driverId: string, @Param('docId') docId: string) {
  ...
  patchDriverVehicle(
    ...
    return this.admin.updateDriverVehicle(id, vehicleId, body, req.salomAdminUserId);
  }
  ...
  rejectDriver(@Param('id') id: string, @Body() body: RejectDriverDto, @Req() req: Request) {
  ...
  patchDriverPayout(
    ...
    return this.admin.updateDriverPayoutDestinationByAdmin(id, body, req.salomAdminUserId);
  }
  ...
  listDriverNews(@Query('take') take?: string, @Query('skip') skip?: string) {
    ...
    const s = skip ? parseInt(skip, 10) : 0;
    return this.admin.listAdminDriverNews(Number.isFinite(t) ? t : 50, Number.isFinite(s) ? s : 0);
  }
  ...
  postSmsTest(@Body() body: TestSmsDto, @Req() req: Request) {
    return this.admin.sendTestSms(body.toPhone, body.body, req.salomAdminUserId);
  }

  @Patch('zones/:id/meter')
  patchZoneMeter(@Param('id') id: string, @Body() body: UpdateZoneMeterDto, @Req() req: Request) {
  ...
  patchPickupRing(
    ...
    return this.admin.patchPickupPricingRing(ringId, body, req.salomAdminUserId);
  }
  ...
  @Patch('operators/:id')
  patchOperator(@Param('id') id: string, @Body() body: UpdateOperatorDto, @Req() req: Request) {
  ...
  activateOperator(@Param('id') id: string, @Req() req: Request) {
    return this.admin.setOperatorStatus(id, UserAccountStatus.ACTIVE, req.salomAdminUserId);
  }

  @Post('operators/:id/suspend')
  @HttpCode(200)
  suspendOperator(@Param('id') id: string, @Req() req: Request) {
    return this.admin.setOperatorStatus(id, UserAccountStatus.SUSPENDED, req.salomAdminUserId);
  }
  ...
  async exportLedger(
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="salom-ledger.csv"');
    return res.send(csv);
  }

  @Get('finance/export/payouts-bank.csv')
  async exportPayoutsBank(@Query('take') take: string | undefined, @Res() res: Response) {
    const t = take ? parseInt(take, 10) : 500;
    const csv = await this.admin.exportPayoutsBankBatchCsv(Number.isNaN(t) ? 500 : t);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="salom-payouts-bank.csv"');
    return res.send(csv);
  }
  ...
  recordAdjustment(@Body() body: RecordLedgerAdjustmentDto, @Req() req: Request) {
  ...
  patchSms(
    ...
    return this.admin.updateSmsTemplate(decodeURIComponent(code), body, req.salomAdminUserId);
  }
