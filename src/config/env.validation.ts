...
export const envValidationSchema = Joi.object({
  ...
  /**
   * Render persistent disk mount (masalan `/var/salom-data`). Berilsa, haydovchi hujjatlari va
   * chempionlar bannerlari `…/driver-uploads` va `…/champions-banners` ostida saqlanadi (deployda yo‘qolmaydi).
   */
  SALOM_UPLOAD_ROOT: Joi.string().optional().allow(''),
  /** To‘liq yo‘l: `SALOM_UPLOAD_ROOT` ustidan ustun. */
  DRIVER_DOC_UPLOAD_DIR: Joi.string().optional().allow(''),
  CHAMPIONS_BANNER_UPLOAD_DIR: Joi.string().optional().allow(''),
});
