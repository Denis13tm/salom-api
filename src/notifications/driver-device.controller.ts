...
import { DriverOnboardingStatus, DriverOperationalStatus, UserAccountStatus } from '@prisma/client';
...
export class DriverDeviceController {
  ...
  async setPresence(@Req() req: Request, @Body() body: SetDriverPresenceDto) {
    ...
    if (!body.online) {
      if (CANT_OFFLINE.includes(d.operationalStatus)) {
        throw new ConflictException('Safar davomida oflayn bo‘lish mumkin emas');
      }
    ...
    if (d.activationCode && !d.appActivatedAt) {
      throw new BadRequestException('Ilovani 12 xonali kod bilan faollashtiring');
    }
    if (d.operationalStatus === DriverOperationalStatus.SUSPENDED) {
      throw new BadRequestException('Haydovchi profili bloklangan — operatorga murojaat qiling');
    }

    if (CANT_OFFLINE.includes(d.operationalStatus)) {
      throw new ConflictException('Safar yoki yetib borgan holat — avval safarni bosing');
    }
    if (
  ...
  setDevice(@Req() req: Request, @Body() body: SetFcmTokenDto) {
    return this.prisma.driver.update({
      ...
      data: {
        ...
        clientDeviceId: body.clientDeviceId === undefined ? undefined : (body.clientDeviceId ?? null),
      },
  ...
  async setPayout(@Req() req: Request, @Body() body: UpdatePayoutDestinationDto) {
    const data: { payoutIban?: string | null; payoutAccountName?: string | null } = {};
    ...
    if (Object.keys(data).length === 0) {
      throw new BadRequestException("Kamida bitta maydon: payoutIban yoki payoutAccountName");
    }
    return this.prisma.driver.update({
      where: { id: req.salomDriverId! },
      data,
