...
const DEFAULT_SMS_TEMPLATES: Record<string, string> = {
  ...
  passenger_no_show: 'Salom Taxi: uchrashuv nuqtasida siz kelmadingiz, buyurtma yopildi.',
  trip_completed: 'Salom Taxi: safar yakunlandi. To‘lov (taxmin): {{grossUzs}} so‘m. Rahmat!',
};
...
export class SmsService {
  ...
  private extractEskizToken(body: unknown): string | null {
    ...
    const msg = o.message;
    if (typeof msg === 'string' && msg.length > 40 && !/\s/.test(msg)) return msg;
    const data = o.data;
  ...
  private async ensureEskizBearer(): Promise<string> {
    ...
    const base = this.config.get<string>('ESKIZ_API_BASE', 'https://notify.eskiz.uz').replace(/\/+$/, '');
    const email = this.config.get<string>('ESKIZ_EMAIL')?.trim();
    const password = this.config.get<string>('ESKIZ_PASSWORD')?.trim();
    if (!email || !password) {
      throw new Error('ESKIZ_EMAIL va ESKIZ_PASSWORD majburiy (SMS_MODE=eskiz)');
    }
    ...
    if (!token) {
      throw new Error('Eskiz auth: token topilmadi (javob formati o‘zgargan bo‘lishi mumkin)');
    }
  ...
  private async sendViaEskiz(toPhone: string, body: string): Promise<string | undefined> {
    const base = this.config.get<string>('ESKIZ_API_BASE', 'https://notify.eskiz.uz').replace(/\/+$/, '');
    const from = this.config.get<string>('ESKIZ_FROM')?.trim();
    if (!from) {
      throw new Error('ESKIZ_FROM majburiy (Eskiz kabinetidagi yuboruvchi nomi / raqam)');
    }
    ...
    try {
      ...
      const id = json.id ?? json.request_id ?? (json.data as Record<string, unknown> | undefined)?.id;
  ...
  async buildCustomerMessage(code: string, vars: Record<string, string>): Promise<string> {
  ...
  async sendToCustomer(
    ...
    if (mode === 'eskiz') {
      const maxRetries = Math.max(0, Number(this.config.get('SMS_HTTP_MAX_RETRIES') ?? 3));
      let lastErr = '';
      for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        try {
          ...
          await this.prisma.sMSLog.update({
            where: { id: row.id },
            data: { status: SmsDeliveryStatus.SENT, sentAt: new Date(), providerId },
          });
          return { logId: row.id };
        } catch (e) {
          lastErr = e instanceof Error ? e.message : String(e);
          this.log.warn(`SMS eskiz urinish ${attempt + 1}/${maxRetries + 1}: ${lastErr}`);
      ...
      await this.prisma.sMSLog.update({
        where: { id: row.id },
        data: { status: SmsDeliveryStatus.FAILED, error: lastErr || 'eskiz_failed' },
      });
    ...
    if (mode === 'http') {
      ...
      if (!url) {
        this.log.warn('SMS_MODE=http lekin SMS_HTTP_URL yo‘q; SENT deb belgilandi');
        await this.prisma.sMSLog.update({
          where: { id: row.id },
          data: { status: SmsDeliveryStatus.SENT, sentAt: new Date() },
        });
      ...
      const maxRetries = Math.max(0, Number(this.config.get('SMS_HTTP_MAX_RETRIES') ?? 3));
      const timeoutMs = Math.max(1000, Number(this.config.get('SMS_HTTP_TIMEOUT_MS') ?? 15_000));
      let lastErr = '';
      for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        try {
          ...
          const extra = this.config.get<string>('SMS_HTTP_HEADERS_JSON')?.trim();
          if (extra) {
            try {
          ...
          const res = await fetch(url, {
            method: 'POST',
            headers,
            body: JSON.stringify({ to: toPhone, body, orderId, logId: row.id }),
            signal: ac.signal,
          });
          clearTimeout(t);
          if (!res.ok) {
          ...
          lastErr = e instanceof Error ? e.message : String(e);
          this.log.warn(`SMS http urinish ${attempt + 1}/${maxRetries + 1}: ${lastErr}`);
          if (attempt < maxRetries) {
      ...
      await this.prisma.sMSLog.update({
        where: { id: row.id },
        data: { status: SmsDeliveryStatus.FAILED, error: lastErr || 'http_failed' },
      });
      this.log.warn(`SMS http yakuniy xato: ${lastErr}`);
      if (opts?.failIfUndelivered) {
        throw new ServiceUnavailableException(`SMS yuborilmadi (HTTP): ${lastErr}`);
      }
      return { logId: row.id };
    }
    await this.prisma.sMSLog.update({
